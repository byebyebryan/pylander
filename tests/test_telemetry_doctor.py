...
def test_parse_sim_log_extracts_final_results_and_profile(tmp_path: Path) -> None:
    ...
    log_path.write_text(
        "\n".join(
            [
                "Mode: headless run",
                "Selector: boost:flat:near:half:0",
    ...
    assert parsed["selector"] == "boost:flat:near:half:0"
...
def test_analyze_compare_produces_crash_and_perf_findings() -> None:
                ...
                "new_crashes": [
                    {
                        ...
                        "repro": {
                            "plot": "uv run python main.py plot boost:flat:mid:half:2 --bot pdg",
                            "sim_trace": "uv run python main.py sim boost:flat:mid:half:2 --bot pdg --freq 1",
                            "sim_profile": "PYLANDER_BOT_PROFILE=1 uv run python main.py sim boost:flat:mid:half:2 --bot pdg --freq 1",
                        },
                    }
                ],
                "candidate_crashes": [
                    {"level": "boost", "scenario": "flat:mid:half", "seed": 2}
                ],
            },
            "compute": {
    ...
    assert any("boost:flat:mid:half:2" in cmd for cmd in report["repro_bundle"])
