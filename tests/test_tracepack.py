...
def test_write_preview_png_draws_actual_and_reference_paths(
    ...
    assert any(
        call["kwargs"].get("color") == "#0057d8" and call["kwargs"].get("linewidth") == 2.0
        for call in calls
    )
    assert any(
        call["kwargs"].get("color") == "#ff8a00" and call["kwargs"].get("linestyle") == "--"
        for call in calls
    )
...
def test_derive_plot_payload_uses_boost_cutoff_for_ballistic_curve(
    ...
    def _fake_ballistic_curve_from_state(*, x, y, vx, vy_up, target_x, target_y, gravity_mag=9.8):  # type: ignore[no-untyped-def]
        observed.update(
            {
                "x": float(x),
                "y": float(y),
    ...
    monkeypatch.setattr(tracepack, "_ballistic_curve_from_state", _fake_ballistic_curve_from_state)
...
def test_trace_recorder_samples_current_frame_time_without_backdating(tmp_path: Path) -> None:
    class _Terrain:
