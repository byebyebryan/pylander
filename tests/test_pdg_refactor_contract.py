...
def test_boost_controller_cuts_when_predicted_terminal_handoff_stalls() -> None:
    bot = _pdg_bot()
    passive = replace(_sensors(), thrust_level=0.6)
    bot.state._elapsed_time_s = 1.20
    bot.state._boost_burn_started = True
    bot.state._boost_burn_start_time = 0.10
    bot.state._boost_release_best_entry_projected_dx_abs = 100.0
    bot.state._boost_release_best_entry_required_accel_ratio = 0.40
    bot.state._boost_release_last_improve_time = 0.0
    ...
    ctx = UpdateContext(
        ...
        nominal_thrust_accel=17.0,
        min_thrust_accel=4.0,
        suggested_stage=FlightStage.BOOST,
    )
    ...
    bot._predict_boost_release_handoff = MethodType(
        lambda self, **_: SimpleNamespace(
            mode="latest_safe",
            entry_projected_dx=99.0,
            required_accel_ratio=0.39,
        ),
        bot,
    )
    bot._run_pdg_stage = MethodType(
        lambda self, **_: BotAction(1.6, 0.0, False),
        bot,
    )
    bot._command_boost_settle = MethodType(
        lambda self, **_: BotAction(0.0, 0.0, False),
        bot,
    )

    result = bot._run_boost_controller(ctx=ctx)

    assert bot.state._boost_cut_latched is True
    assert bot.state._boost_release_mode == "latest_safe"
    assert bot.state._boost_release_predicted_terminal_entry_projected_dx == pytest.approx(
        99.0
    )
    assert result.next_stage is None
    assert result.action is not None
    assert result.action.target_thrust == pytest.approx(0.0)
    assert result.action.status == "pdg settle/boost dx"


def test_boost_controller_keeps_boosting_while_predicted_terminal_handoff_improves() -> (
    None
):
    bot = _pdg_bot()
    passive = replace(_sensors(), thrust_level=0.6)
    ...
    bot._predict_boost_release_handoff = MethodType(
        lambda self, **_: SimpleNamespace(
            mode="latest_safe",
            entry_projected_dx=92.0,
            required_accel_ratio=0.36,
        ),
        bot,
    )
    bot._run_pdg_stage = MethodType(
    ...
    assert bot.state._boost_release_best_entry_projected_dx_abs == pytest.approx(92.0)
    assert bot.state._boost_release_best_entry_required_accel_ratio == pytest.approx(
        0.36
    )
    assert result.next_stage is None
    assert result.action is not None
    assert result.action.target_thrust == pytest.approx(1.6)
