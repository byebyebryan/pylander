"""Setup bot: hard side-burn setup, then hand off to coast."""
...
SETUP_COURSE = replace(
    COAST_COURSE,
    # Setup handoff often carries high lateral speed; start braking earlier.
...
_SETUP_BEHAVIORS: dict[str, tuple[DropPolicy, CoastCourseConfig, LaunchSetupConfig]] = {
    "setup": (SETUP_POLICY, SETUP_COURSE, LaunchSetupConfig()),
}


def resolve_setup_behavior(
    behavior: str,
...
def list_setup_behavior_names() -> tuple[str, ...]:
    return tuple(sorted(_SETUP_BEHAVIORS))


class SetupBot(CoastBot):
    def __init__(self, behavior: str = "setup") -> None:
    ...
    def set_behavior(self, behavior: str) -> None:
        key, policy, cfg, setup_cfg = resolve_setup_behavior(behavior)
        self._policy = policy
        ...
        try:
            ...
            if passive.state == "flying" and self._setup_handoff_done:
                ...
                if not action.status:
                    action.status = "setup:handoff_coast"
    ...
    def _guidance(
        ...
        *,
        ...
        setup_guidance = apply_setup_guidance(
            current_guidance,
            self._course_cfg,
            self._setup_cfg,
            vx=passive.vx,
    ...
    def get_evaluation_snapshot(self) -> dict[str, Any] | None:
        if self._handoff_snapshot is None:
            return {"kind": "setup", "handoff_done": bool(self._setup_handoff_done)}
        return dict(self._handoff_snapshot)

    def _coast_burn_command(self, *, guidance: GuidanceTargets, **kwargs):
        # Keep coast burn state dormant during setup-sideburn setup.
    ...
    def _horizontal_controller(
        ...
        if (
            ...
            and guidance.phase == "setup_sideburn"
        ):
    ...
    def _allocate_controls(
        ...
        *,
        ...
        # Setup gets one setup burn attempt; after it ends, stop thrusting and
        ...
        if vertical_mode != "setup_sideburn":
