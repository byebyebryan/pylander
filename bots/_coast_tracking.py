"""Shared coast-style lateral tracking and correction helpers."""
...
from bots._bot_math import clamp, resolve_behavior
from bots._drop_guidance import BASE_DROP_POLICY, DropPolicy
...
class CoastCourseConfig:
    cone_dx_base: float = 8.0
    cone_dx_per_alt: float = 0.06
    cone_dx_max: float = 52.0
    correction_vx_min: float = 1.8
    correction_vx_per_excess: float = 0.12
    correction_vx_per_alt: float = 0.012
    correction_vx_high_alt_cap: float = 9.8
    correction_vx_low_alt_cap: float = 3.2
    correction_vx_low_alt_threshold: float = 32.0
    correction_delta_vx_gain: float = 1.0
    ...
    coast_burst_min_dx: float = 14.0
    coast_burst_min_delta_vx: float = 1.4
    coast_hold_descending_vy_max: float = 0.0
...
def apply_coast_guidance(
    ...
    *,
    ...
    current_vy_up = float(vy_up) if vy_up is not None and math.isfinite(vy_up) else 0.0
    vx_delta_needed = projected_dx / max(0.5, projection.t_fall)
    vx_needed = current_vx + vx_delta_needed
    vx_need_mag = abs(vx_delta_needed)
    ...
    if abs_projected_dx > cone_limit:
        ...
        vx_sp = current_vx + clamp(
            cfg.correction_delta_vx_gain * vx_delta_needed,
            -correction_delta_vx_cap,
            correction_delta_vx_cap,
        )
    ...
    if guidance.vertical_mode == "terminal_burn":
        ...
        if abs(guidance.dx) <= cfg.lateral_terminal_zero_vx_dx and alt <= cfg.lateral_terminal_zero_vx_alt:
            vx_sp = clamp(
                vx_sp,
    ...
    if (
        ...
        and current_vy_up <= cfg.coast_hold_descending_vy_max
        and abs_projected_dx > coast_limit
        and abs_projected_dx >= cfg.coast_burst_min_dx
        and abs(vx_delta_needed) >= cfg.coast_burst_min_delta_vx
...
def lateral_tracking_command(
    ...
    *,
    ...
    t_go = clamp(projection.t_fall, tgo_min, tgo_max)
    projected_dx = projection.projected_dx
    abs_projected_dx = abs(projected_dx)
    delta_vx_cap = correction_vx_cap(safe_alt, cfg)
    delta_vx_needed = projected_dx / max(0.5, t_go)
    vx_track = vx + clamp(
        cfg.correction_delta_vx_gain * delta_vx_needed,
        -delta_vx_cap,
        delta_vx_cap,
    )
    track_weight = clamp(
        abs_projected_dx / max(1e-3, cfg.lateral_track_dx_full),
        cfg.lateral_track_min_weight,
        1.0,
    )
    if projection.used_sensor and abs_projected_dx > 8.0:
        track_weight = clamp(
    ...
    if abs_dx <= cfg.lateral_zero_vx_dx and safe_alt <= cfg.lateral_zero_vx_alt:
        vx_target = clamp(vx_target, -cfg.lateral_zero_vx_cap, cfg.lateral_zero_vx_cap)
    vx_err = vx_target - vx
    pos_term = clamp(
...
COAST_COURSE = replace(
    CoastCourseConfig(),
    cone_dx_base=5.0,
    cone_dx_per_alt=0.03,
    cone_dx_max=28.0,
    correction_vx_min=3.0,
    correction_vx_per_excess=0.24,
    correction_vx_per_alt=0.015,
    correction_vx_high_alt_cap=14.0,
    correction_vx_low_alt_cap=6.0,
    correction_vx_low_alt_threshold=33.0,
    correction_delta_vx_gain=1.8,
    terminal_burn_correction_vx_floor=4.8,
    coast_enter_scale=1.0,
    coast_burst_min_dx=14.0,
    coast_burst_min_delta_vx=1.5,
    coast_hold_descending_vy_max=-0.8,
    coast_min_altitude=40.0,
    coast_descent_floor=1.8,
    ...
    lateral_track_min_weight=0.35,
    lateral_track_dx_full=48.0,
    sensor_track_weight_boost=0.14,
