...
def _metrics_from_compare(compare: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "fuel_mean_primary_delta": to_float(
            summary_delta.get("fuel_mean_primary"), 0.0
        ),
        ...
        "observation_regressions": len(
            list(observation_crash.get("new_crashes") or [])
        ),
        "notable_global_regression": bool(
            global_block.get("notable_regression", False)
        ),
    }
...
def _run_validation(
    ...
    selectors = [
        str(s).strip() for s in payload.get("selectors") or [] if str(s).strip()
    ]
    if not selectors:
        raise ValueError(
            "selectors must be non-empty when execute-validation is enabled"
        )
...
def generate_branch_report(
    payload: dict[str, Any], *, execute_validation: bool = False
) -> dict[str, Any]:
    ...
    normalized_summary = {
        ...
        "fuel_mean_primary_delta": to_float(
            summary.get("fuel_mean_primary_delta"), 0.0
        ),
        ...
        "observation_regressions": max(
            0, to_int(summary.get("observation_regressions"), 0)
        ),
        "notable_global_regression": bool(
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Execute one Pylander arena branch and emit a normalized report"
    )
    ...
    report = generate_branch_report(
        payload, execute_validation=bool(args.execute_validation)
    )
