---
name: pylander-strategy-orchestrator
description: Orchestrate parallel strategy experiments, compare standardized branch reports, and pick a winner for tuning.
---

# Pylander Strategy Orchestrator

Use this skill when there are multiple plausible strategy directions and no clear winner yet.

Script-backed status:
- executor: `.agents/skills/pylander-strategy-orchestrator/scripts/run_strategy_arena.py`
- output contract: `.agents/skills/contracts/arena_scoreboard.v1.json`

## Command

`uv run python .agents/skills/pylander-strategy-orchestrator/scripts/run_strategy_arena.py --input <input_json> --output <output_json> --no-execute-workers`

Optional worker execution for branches that provide `worker_input` instead of `report_path`:

`uv run python .agents/skills/pylander-strategy-orchestrator/scripts/run_strategy_arena.py --input <input_json> --output <output_json> --execute-workers`
...
- `winner`: hand off to `pylander-tune-routing-planner`
...
4. `next_step_handoff` for `pylander-tune-routing-planner`
