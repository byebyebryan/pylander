...
_WORKER_SCRIPT = (_REPO_ROOT / ".agents" / "skills" / "pylander-arena-branch-runner" / "scripts" / "run_arena_branch.py").resolve()
...
def _resolve_report_for_branch(
    *,
    ...
    raise ValueError("Each strategy branch must provide report_path, inline_report, or executable worker_input")


def run_strategy_arena(payload: dict[str, Any], *, execute_workers: bool) -> dict[str, Any]:
    ...
    if winner is None:
        outcome = "no_winner"
        winner_branch_id = ""
        no_winner_reason = "No branch passed hard gates and promote/iterate criteria"
        handoff = {
            "target_skill": "none",
            "payload": {
                "reason": no_winner_reason,
                "recommended_action": "redesign_strategy_candidates",
            },
        }
    else:
        outcome = "winner"
        ...
        no_winner_reason = ""
        handoff = {
            "target_skill": "pylander-tune-routing-planner",
            "payload": {
                "strategy_winner_ref": winner_branch_id,
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run Pylander strategy arena ranking")
    ...
    report = run_strategy_arena(payload, execute_workers=bool(args.execute_workers))

    out_path = write_json(args.output, report)
    artifact_path = (
        _REPO_ROOT / "outputs" / "arena" / str(report["arena_id"]) / "scoreboard.strategy.json"
    ).resolve()
    write_json(artifact_path, report)
    print(f"# strategy_arena\njson={out_path}\nartifact={artifact_path}")
