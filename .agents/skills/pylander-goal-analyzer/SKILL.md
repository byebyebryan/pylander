...
- `uv run python .agents/skills/pylander-benchmark-runner/scripts/run_cached_benchmark.py --mode focused --selectors <selector_scope...> --seed-spec 0-4 --baseline-ref <baseline_ref> --bot <bot>`
...
5. Hand off to `pylander-strategy-orchestrator` with candidate bundle.

Handoff command template:

`uv run python .agents/skills/pylander-strategy-orchestrator/scripts/run_strategy_arena.py --input <arena_input_json> --output <arena_output_json> --no-execute-workers`
...
4. `next_step_handoff`: ready input payload for `pylander-strategy-orchestrator`
